...
def has_no_deletes(t: unittest.TestCase, preview: auto.PreviewResult) -> None:
...
def has_no_changes(t: unittest.TestCase, preview: auto.PreviewResult) -> None:
    change_summary = ChangeSummary(preview.change_summary)
    unexpected_ops = change_summary.where_op_not_equals(OpType.SAME)
    if len(unexpected_ops) > 0:
        t.fail(f"expected no changes, got {unexpected_ops}\n{preview.stdout}")

def has_no_replacements(t: unittest.TestCase, preview: auto.PreviewResult) -> None:
    change_summary = ChangeSummary(preview.change_summary)
