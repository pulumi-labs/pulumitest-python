...
def has_no_deletes(t: unittest.TestCase, up: auto.UpResult) -> None:
    resource_changes = up.summary.resource_changes
...
def has_no_changes(t: unittest.TestCase, up: auto.UpResult) -> None:
    resource_changes = up.summary.resource_changes
    if resource_changes is None:
        return
    change_summary = ChangeSummary(resource_changes)
...
def has_no_replacements(t: unittest.TestCase, up: auto.UpResult) -> None:
    resource_changes = up.summary.resource_changes
    if resource_changes is None:
        return
    change_summary = ChangeSummary(resource_changes)
